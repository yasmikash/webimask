...
import { fetchHeartRateHistory } from "../../services/api";
...
export default function AnalysisTable() {
  ...
  const fetchHeartRateHistoryAsync = async () => {
    const data = await fetchHeartRateHistory();
    setData(data);
  };

  React.useEffect(() => {
    fetchHeartRateHistoryAsync();
  }, []);
      ...
      >
        <Table stickyHeader sx={{ width: "100%" }} aria-label="simple table">
          ...
          <TableBody>
            {data.map((row) => (
              <TableRow
                key={row.DesktopDatePicker}
                sx={{ "&:last-child td, &:last-child th": { border: 0 } }}
              >
                <TableCell component="th" scope="row">
                  {moment(new Date(row.date)).format("YYYY-MM-DD HH:mm A")}
                </TableCell>
                <TableCell align="right">{row.heartRate} bpm</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
