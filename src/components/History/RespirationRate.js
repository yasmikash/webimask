...
import { fetchRespiratoryHistory } from "../../services/api";
...
export default function AnalysisTable() {
  ...
  const fetchRespiratoryHistoryAsync = async () => {
    const data = await fetchRespiratoryHistory();
    setData(data);
  };

  React.useEffect(() => {
    fetchRespiratoryHistoryAsync();
  }, []);
          ...
          <TableBody>
            {data.map((row) => (
              ...
              >
                ...
                <TableCell align="right">{row.bpm} bpm</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
